                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500 mb-3">
                              {item.product.categories && item.product.categories.length > 0 && item.product.categories.some((cat: any) => typeof cat === 'object' && cat.name)
                  ? item.product.categories.filter((cat: any) => typeof cat === 'object' && cat.name).map(cat => cat.name).join(', ')
                  : item.product.category.name
                } • Sizes: {item.product.sizes.map((s: { size: string }) => s.size).join(', ')} • Color: {item.product.color}
                            </p>
                          </div>
                          <button
